...
class FoundBook(Toplevel):
    ...
    def frame(self):
        # Frame Picture
        img = ImageTk.PhotoImage(file='/Users/rosannagamal/assignment_two/HomePage/bookpage.png')
        Label(self, image=img, bg='#061A1D').place(x=0, y=0)

    def book_cover(self):
        # Cover Picture
        file = os.path.join('/Users/rosannagamal/assignment_two/book_covers', self.book.file)
        img = ImageTk.PhotoImage(file=file)
        Label(self, image=img, bg='#061A1D').place(x=58, y=68)
